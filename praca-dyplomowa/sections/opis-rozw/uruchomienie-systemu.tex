...
Przy wdrażaniu systemu najszybciej i najbezpieczniej jest skorzystać z metod opisanych w rozdziale \ref{system_startup.client_obtaining} i \ref{system_startup.containers}.
...
\begin{itemize}
	...
	\item \texttt{VagrantboxUri} - identyfikator szablonowej maszyny wirtualnej. Musi ona spełniać szereg wymogów opisanych w rozdziale \ref{system_requirements.vagrant_box}. W przeciwnym wypadku system nie będzie w stanie uruchomić takiego szablonu. Aplikacja nie zadziała prawidłowo bez ustawionej wartości parametru.
...
Przy uruchomieniu kontenera należy ustawić zmienną środowiskową \texttt{API\_URL} zgodnie z opisem w rozdziale \ref{system_startup.admin_panel_conf}.
